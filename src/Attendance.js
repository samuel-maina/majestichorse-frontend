...
class Attendance extends React.Component {
    ...
    async componentDidMount() {
        ...
        const response = await axios.get("http://localhost:8080/api/v1/attendance/"+this.props.match.params.id)
                .then((response) => {
   ...
   async addToGroup(phone){
       await axios.post("http://localhost:8080/api/v1/attendance/"+phone+"/"+this.props.match.params.id)
                .then((response) => {
                    console.log(response);
                    this.setState({res: ""});
                }).catch((err) => {
    ...
    render() {
        ...
        return(
                <div class="content">
                    ...
                    <div class="Roboto margin-top-sm">{this.state.group.sundayDate} <div class="border-bottom-crimson bg-crimson margin-top-xssm"></div></div>
                    <div class="float-right"><Link to={'/groups/add/'+this.state.group.id}><div class="btn-variant-7 border-rad-md fixed-float"><span class="material-symbols-outlined margin-sm">
                                person_add
                                </span></div></Link><div class="fixed-float-3 ">
                                {this.state.congregants.length===0?<div class="flex-vertical margin-top-md center">Group has no members</div>:<div class="three-per-row w-80 float-left">
